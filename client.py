...
def main():
    ...
    try:
        ...
        _thread.start_new_thread(send_thread, (clientSock, args))
        _thread.start_new_thread(recv_thread, (clientSock, args))
...
def send_thread(_clientSock, args):
    while True:
        input_str = input('>>> ')
        if input_str == 'exit':
...
def recv_thread(_clientSock, args):
    while True:
